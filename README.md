...
python -m tests.test_sandbox          # 코드 실행 샌드박스 98건
...
**총 124건 + 전수 감사 통과.** 그 밖에 구현 중 확인한 사항:
...
검증: `python -m tests.test_sandbox` — 공격 **72종 전부 차단**, 정상 분석 코드 **26종 전부 통과**.
실제 앱으로 15개 질문을 돌려 **실패 0건 · 재시도 0건**을 확인했습니다.

### 남는 한계 — 가용성 (정직하게 밝힙니다)

**데이터 유출과 임의 코드 실행은 막았지만, 앱을 느리게 하거나 죽이는 것은 완전히 막지
못했습니다.** 적대적 검토에서 확인된 내용입니다.

```python
n = 1000000
x = [0] * n * n      # 리터럴은 전부 작아서 정적 검사를 통과한다 → 1조 원소
```

정적 분석은 **변수에 담긴 값을 볼 수 없습니다.** 리터럴 크기 제한은 `[0] * (10**8)`
같은 직접적인 형태만 잡습니다. 또 파이썬은 스레드를 강제 종료할 수 없어, 15초 상한이
지나도 폭주한 코드는 계속 CPU 를 씁니다. 상한은 화면이 멈추지 않게 할 뿐입니다.

**올바른 해법은 별도 프로세스 + OS 자원 상한(`RLIMIT_AS`/`RLIMIT_CPU`)이고,
타임아웃 시 실제로 죽이는 것입니다.** 구현을 시도했지만 `forkserver` 가 이 환경에서
실패했고, `spawn` 은 자식이 `main.py` 를 다시 import 하면서 Streamlit 앱 전체를
재실행할 위험이 있었습니다. 제출 직전에 검증되지 않은 재작성을 넣는 것보다
한계를 명시하는 편이 낫다고 판단해 되돌렸습니다.

**영향 범위**: 가용성만 해당합니다. 데이터가 새거나 코드가 실행되지는 않습니다.
악의적 프롬프트로 앱을 느리게 하거나 재시작시킬 수 있습니다.
실제 트래픽을 받는 서비스라면 위 프로세스 격리를 먼저 구현해야 합니다.

그 밖에 같은 프로세스에서 `exec` 하는 구조상, 허용한 pandas 메서드에 새로운
파일 I/O 인자가 생기면 다시 위험해질 수 있습니다. `tests/test_allowlist_audit.py`
가 이를 자동으로 감지하도록 해 두었습니다.
