...
python -m tests.test_sandbox   # 코드 실행 샌드박스 79건
```

**총 105건 전부 통과.** 그 밖에 구현 중 확인한 사항:
...
- **디스패처 문자열 검사** — pandas 는 `agg`/`apply`/`transform`/`pipe`/`map` 에
  ...
  이제 이 메서드들의 **이름 자리**에는 다음 셋만 올 수 있습니다.
  집계 함수 이름인 **문자열 상수**, **lambda**, 또는 **함수로 정의된 이름**.
  변수·f-string·문자열 연결처럼 값을 정적으로 알 수 없는 표현식은 거부합니다
  (`m = 'to_csv'; df.agg(m, 0, path)` 로 실제 파일이 쓰였습니다 — 모르면 거부).
  `agg({'열': 'mean'})` 의 딕셔너리 키는 열 이름이므로 검사에서 제외합니다.
...
검증: `python -m tests.test_sandbox` — 공격 **57종 전부 차단**, 정상 분석 코드 **22종 전부 통과**.
