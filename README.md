...
# 그래프 구조 검증 (노드 수 · 조건부 엣지 수)
python -m tests.test_graph
```

구현 중 확인한 사항:

- 분기 함수 19건 + 그래프 구조 4건, 총 23건 단위 테스트 전부 통과
- 6개 라우팅 경로 실제 LLM 호출로 종단 확인
- CE3 자기치유 루프 — 잘못된 열 이름으로 실패시킨 뒤 재시도가 코드를 고쳐 복구하는 것을 확인
- CE4 웹 폴백 · CE6 재작성이 실제 질문에서 발화하는 것을 확인

## 9. 한계와 주의사항

- **`utils.run_code()` 는 LLM이 생성한 파이썬 코드를 `exec` 으로 실행합니다.** 로그 조회·차트 생성을 위한 구조이지만 임의 코드 실행 경로이므로, 저장소와 배포 앱을 private 으로 유지하고 URL 공유 범위를 제한하세요.
